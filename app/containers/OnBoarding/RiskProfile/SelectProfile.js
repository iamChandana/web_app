...
class SelectProfile extends React.PureComponent {
  ...
  handleAccept() {
    this.props.saveRiskScore({ riskProfileType: this.state.selected });
  ...
  saveRiskProfile() {
    ...
    if (riskScore.riskProfileType.toLowerCase() !== this.state.selected.toLowerCase()) {
      ...
      const modalMessage = `Your risk profile is ${riskScore.riskProfileType} but you have selected as ${
        this.state.selected
      } Risk Profile.`;
      this.setState({
        open: true,
  ...
  return() {
    this.props.history.replace('/onboarding/risk-profile/result');
  }
  render() {
