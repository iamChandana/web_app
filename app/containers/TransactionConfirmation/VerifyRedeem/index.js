...
import { verifyRedeemRequest } from '../actions';
...
export class VerifyRedeem extends React.PureComponent {
  componentDidMount() {
    const {
      history,
      location: { search },
      handleVerifyRedeemRequest,
    } = this.props;
    ...
    handleVerifyRedeemRequest({ token, qStrMsgFromEmail });
  }
  ...
  render() {
    ...
    return (
      <Grid spacing={24} container>
        ...
        <Grid item xs={12}>
          <RedeemSummary data={verifyTransactionApiResponse} />
        </Grid>
      </Grid>
    );
  }
...
VerifyRedeem.propTypes = {
  history: PropTypes.object.isRequired,
  location: PropTypes.object.isRequired,
  handleVerifyRedeemRequest: PropTypes.func.isRequired,
...
const mapDispatchToProps = (dispatch) => ({
  handleVerifyRedeemRequest: (payload) => dispatch(verifyRedeemRequest(payload)),
});
